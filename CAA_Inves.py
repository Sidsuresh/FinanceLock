...
def open_caa_inves():
	...
	def next_but():
		...
		if len(e_inv.get()) == 0 or e_inv.get() == "Entry should be filled":
			e_inv.delete(0, END)
			e_inv.insert(0, "Entry should be filled")
			chk_list.append(0)
		else:
			chk_list.append(1)
		...
		if chk_cnt == 1 and t1:
			...
			data_good = {'name': name,'dob': e_dob.get(), 'gender': click_gen.get(), 'proof_id': click_pid.get(), 'phone_no': e_pno.get(), 'employed': click_emp.get(), 'occupation': e_ocp.get(), 'doc_num': e_doc.get(), 'issue_date': e_idate.get(), 'expiry_date': e_edate.get(), 'email': e_email.get(), 'invest_balance': e_inv.get(), 'permanent_address': perm_add, 'present_address': pr_add}
	...
	my_canvas.create_text(90, 375, text = "Invested Money: ", font = ("Arial Rounded MT Bold", 12), fill = "steel blue")
	e_inv = Entry(top_cain, width = 25, borderwidth = 2)
	#e_sal.insert(0, "Salary")
	e_inv_win = my_canvas.create_window(250, 375, window = e_inv)
